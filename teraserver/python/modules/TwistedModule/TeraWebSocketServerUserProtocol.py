...
class TeraWebSocketServerUserProtocol(TeraWebSocketServerProtocol):
    ...
    def onMessage(self, msg, binary):
        ...
        #             #     session = TeraSession.get_session_by_uuid(join_session_reply.session_uuid)
        #             #     if session is None:
        #             #         print('TeraWebSocketServerUserProtocol - Join Session Reply - Unknown Session UUID')
        #             #         return
        #             #
        #             #     # Forward reply to everyone in the session
        #             #     for user_uuid in [user.user_uuid for user in session.session_users]:
        #             #         if user_uuid != self.user.user_uuid:
        #             #             self.send_event_message(join_session_reply, 'websocket.user.' + user_uuid + '.events')
        #             #
        #             #     for part_uuid in [part.participant_uuid for part in session.session_participants]:
        #             #         self.send_event_message(join_session_reply, 'websocket.participant.' + part_uuid +
        #             #                                 '.events')
        #             #
        #             #     for device_uuid in [device.device_uuid for device in session.session_devices]:
